...
**Guard:** Inline comment block on the `LSUIElement` key in `MacosWidgetsStatsFromWebsite/Apps/MainApp/Info.plist` explicitly documents WHY this is false and that the widget budget is unaffected — re-adding `LSUIElement=true` without removing the comment + understanding the consequences is a regression smell. Also: after install, `Stats Widget from Website.app` should show in the Dock with a clickable icon, and double-clicking it from Finder should bring its prefs window to the foreground. If the Dock icon doesn't appear, LSUIElement is still true somewhere (check the built `Contents/Info.plist`).
---
