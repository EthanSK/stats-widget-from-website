...
# .xcodeproj is gitignored (regenerated by xcodegen from project.yml — not the
# source of truth, no point versioning the generated tree). Skip the add. If
# the gitignore policy ever changes, this guard transparently picks it back up.
if [[ -d "$REPO_ROOT/MacosWidgetsStatsFromWebsite.xcodeproj" ]] \
    && ! git -C "$REPO_ROOT" check-ignore -q "$REPO_ROOT/MacosWidgetsStatsFromWebsite.xcodeproj"; then
